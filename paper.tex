...
% --- inserted: cross-loop representational dynamics (P0 probe) ---
...
Categorizing the per-loop output revisions by gold-token type (GSM8K,
$N\!=\!1904$ digit / $1418$ operator tokens) reveals \emph{where} depth is spent.
At the converged tail ($6\!\to\!5$) the revisions are uniform across content
tokens --- per-category flip rate $0.13$ (digit), $0.12$ (operator), $0.11$
(word), with only punctuation spared ($0.06$) --- so the last loop does
undifferentiated, low-stakes polish. But sweeping \emph{all} adjacent boundaries
shows this is the tail of a front-loaded, reasoning-specialized process: at the
first refinement boundary ($3\!\to\!2$) the model revises digits and logical
connectives at $0.39$ and $0.34$ --- $1.7\times$ and $1.5\times$ the generic-word
rate ($0.23$) --- and this enrichment decays monotonically to uniform by loop~6
(digit $0.39\!\to\!0.27\!\to\!0.18\!\to\!0.13$). The recursion thus spends its
\emph{early} depth disproportionately on reasoning-relevant tokens and its
\emph{late} depth on lexical settling, mirroring the representational
contraction. (The digit signal is robust at $N\!\approx\!1.9$k; the connective
trend rests on $N\!=\!128$ and the operator enrichment is marginal.) Two scopes
bound the claim: flip rate is depth-\emph{sensitivity}, not correctness (it marks
where the output moves between loops, not where it becomes right), and it is
teacher-forced, not free-running generation. The per-loop auxiliary LM heads
(\S\ref{sec:aux-heads}), which train every depth to be decodable, are the
mechanism by which these intermediate revisions are well-formed predictions.
All figures are on the current mid-trained base (GSM8K ppl $11.9$,
in-distribution); the generative behaviour, the post-SFT/RL picture, and whether
the reasoning-on$>$off delta survives fewer loops remain to be measured --- a
compute saving that erodes that delta is rejected regardless of throughput.
