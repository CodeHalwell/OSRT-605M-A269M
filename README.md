...
```
Embedding (65,536 × 1536, tied with LM head)   : 100,690,944   (16.6 %)
Attention × 3 blocks (qkv + out_proj)          :  28,321,152   (4.7 %)
Shared experts × 3 (SwiGLU h=2816)             :  38,928,384   (6.4 %)
Routed experts: 3 × 8 × SwiGLU h=3840          : 424,673,280   (top-2 of 8 active ≈ 17.5 %)
HRA adapters (rank 256, 18 injection points)   :  14,155,776   (always trained)
Router + loop_emb + norms + sink               :     ~44,547

Total physical params                          : 606,814,083 (~607M)
Active per token                               : 288,309,123 (~288M, 47.5 %)
Effective compute per token (× 6 loops)        : ~2.5 B FLOPs-equivalent
```

Generated by `scripts/compute_budget.py` against
`OSRT_605M_A288M` in `src/osrt/presets.py`. Re-run to verify.
...
| Active params per token | 700M | **288M** (top-2 of 8 = 25% routing density; see ARCHITECTURE.md §2.1) |
...
The **607M physical / 288M active** (locked 2026-06-08, generated by
