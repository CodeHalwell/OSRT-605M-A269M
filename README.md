...
training run. The design decisions below are now realised in code; where a
...
```
Embedding (65,536 × 1536, tied with LM head)   : 100,690,944   (16.7 %)
Attention × 3 blocks (q + kv_down + v_from_k + out_proj)
                                               :  17,308,032   (2.9 %)
Shared experts × 3 (SwiGLU h=2816)             :  38,928,384   (6.5 %)
Routed experts: 3 × 8 × SwiGLU h=3840          : 424,673,280   (top-2 of 8 → ~25 % active)
...
Generated by `PYTHONPATH=src python scripts/compute_budget.py`
(reports the canonical `OSRT_605M_A288M` preset in `src/osrt/presets.py`).
Re-run to verify. NOTE: "605M/A288M" in the preset/repo names predates
the corrected count — the instantiated model is 601M physical / 278M
active. "600M" in the model name rounds the physical count.
...
- **8 routed experts (not 12)** for denser routing — top-2 of 8 =
  25% active vs top-2 of 12 = 16.7%. At 605M scale this prioritizes
  per-token capacity over per-expert specialization (the DeepSeek
  "many tiny experts" thesis is more compelling at 16B+ scale).
- **Recursive 6-loop trick** delivers ~1.8× LFM2-700M's FLOPs per
  token from ~41% of LFM2's active params
...
| Routed experts per block | 8 | **8** (kept) | Denser routing (top-2 of 8) preserves per-token capacity at 605M scale; "many tiny experts" deferred to 16B+ |
| Routed expert hidden | 2048 | **3,840** | Each expert wider (since fewer of them) to absorb capacity |
| Shared expert hidden | 4096 | **2,816** | Trial-and-error to fit 605M target; revisit at GPU phase |
...
The **601M physical / 278M active** (generated by
`scripts/compute_budget.py`) is in the **same compute class as
Phi-3-mini (3.8B dense)** thanks to the 6× recurrence multiplier — so
we punch heavier than the dense-equivalent baseline.
...
- **Not a critique of v5.** v5 was the path that taught us this. The
  363M model exists; the lessons exist. Both are valuable.
- **Not exhaustive on training config.** The architecture is built
  (`src/osrt/`), but many training-time decisions (exact LR schedules,
  batch sizes per phase, data-mix ratios) are still being tuned and
  live in `src/osrt/train_config.py` / `presets.py`, not here. This
  doc fixes the architecture-level choices; the code is the source of
