...
def _latest_hf(repo: str, prefix: str, ckpt_dir: Path) -> tuple[Path | None, int]:
    ...
    try:
        ...
        try:
            shutil.copy2(
                hf_hub_download(repo, ema_name, repo_type="model"),
                ckpt_dir / f"{prefix}_ema.pt",
            )
            print(f"  also pulled {ema_name} -> EMA sidecar", flush=True)
        except Exception:
            print(
                "  no matching EMA sidecar on HF; shadow restarts from the "
                "resumed weights",
                flush=True,
            )
        print(
            "  NOTE: optimizer state is local-only, so AdamW moments are COLD "
            "(~20-step rebuild at beta2=0.95)",
            flush=True,
        )
        return dest, best_step
    except Exception as e:  # noqa: BLE001 — a failed scan must not kill the run
        print(
            f"HF resume scan failed ({type(e).__name__}: {e}); "
            f"falling back to the base checkpoint",
            flush=True,
        )
        return None, -1
...
def main() -> int:  # noqa: PLR0915
    ...
    ap.add_argument("--base-ckpt", default="osrt_v5_sft_v4_soup_1200_1400_1600_1800.pt")
    ...
    ap.add_argument(
        "--micro-batch", type=int, default=4, help="sequences per log-prob forward"
    )
    ...
    print(
        f"lr: peak {cfg.peak_lr:.3e}  hra {cfg.hra_lr:.3e}  "
    ...
    if resume_path is None:
        ...
        if not local_base.exists():
            ...
            src = hf_hub_download(
                args.hf_repo or "HallD/osrt-v6-ckpt", args.base_ckpt, repo_type="model"
            )
            local_base = Path(src)
    ...
    hra_params = [
        p for n, p in model.named_parameters() if "adapters_a" in n or "adapters_b" in n
    ]
    ...
    if args.ema_decay > 0:
        ...
        if start_step > 0 and ema_path.exists():
            ...
            if (
                int(eck.get("source_step", -1)) == start_step
                and float(eck.get("ema_decay", -1)) == args.ema_decay
            ):
                ...
                print(
                    f"restored EMA (decay {args.ema_decay}, "
                    f"{ema_updates} updates) from step {start_step}",
                    flush=True,
                )
            else:
                print(
                    f"EMA sidecar is step {eck.get('source_step')}/decay "
            ...
            del eck
        print(
            f"weight EMA: decay {args.ema_decay}, {len(ema_state)} tensors, "
            f"fp32 shadow (passive; never samples, never trains)",
            flush=True,
        )

    print(
        f"full-parameter GRPO: {sum(p.numel() for p in base_params):,} base "
        f"+ {sum(p.numel() for p in hra_params):,} HRA",
        flush=True,
    )
    ...
    if start_step > 0 and opt_path.exists():
        try:
            ...
            if int(ock.get("step", -1)) == start_step:
                ...
                print(
                    f"optimizer sidecar is step {ock.get('step')}, "
            ...
            print(
                f"optimizer restore failed ({type(e).__name__}: {e}); "
                f"continuing with cold moments",
                flush=True,
            )
    elif start_step > 0:
    ...
    if args.compile:
        print(
            "compiling policy (cold trace is minutes; helps every step after)...",
            flush=True,
        )
    ...
    print(
        f"{len(prompts_all)} prompts from {args.prompts} (gold validated)", flush=True
    )
    ...
    for step in range(start_step, cfg.total_steps):
        ...
        if step % cfg.log_interval == 0 or step == start_step:
            ...
            print(
                f"step {step:>5d}/{cfg.total_steps} | loss {loss_val:.4f} | "
        ...
        if step % cfg.sample_print_interval == 0:
            print(
                f"  ---- rollouts @ step {step} (T={cfg.temperature}) ----", flush=True
            )
            for r in groups[0][:3]:
                print(
                    f"  [{'OK ' if r.correct else 'x  '}] rw {r.reward:+.2f} "
        ...
        if step > 0 and step % args.ckpt_interval == 0:
            ...
            torch.save(
                {"step": step, "optimizer_state_dict": optimizer.state_dict()}, opt_out
            )
            if ema_state is not None:
                ...
                print(
                    f"  -> saved EMA sidecar ({ema_updates} updates, "
                    f"{w0:.1%} residual weight on the base)",
                    flush=True,
                )
                if args.hf_repo and step % args.ema_push_interval == 0:
                    ...
                    try:
                        ...
                        HfApi().upload_file(
                            path_or_fileobj=str(named),
                            path_in_repo=named.name,
                        ...
                        print(
                            f"  EMA push failed ({type(e).__name__}: {e})", flush=True
                        )
    ...
    torch.save({"step": cfg.total_steps, "model_state_dict": model.state_dict()}, final)
    ...
    if args.hf_repo:
        ...
        for f in to_push:
            try:
                ...
                print(
                    f"  push failed for {f.name} ({type(e).__name__}: {e})", flush=True
                )
    return 0
