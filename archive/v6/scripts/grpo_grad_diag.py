...
def _cosine(
    a: dict[str, torch.Tensor], b: dict[str, torch.Tensor], which: str = "all"
) -> float:
    dot = na = nb = 0.0
...
def _backward_policy(
    ...
    for i in range(0, n, micro_batch):
        ...
        pol = _seq_logprobs(
            model, batch, p_lens, s_lens, real_vocab, True, temperature=cfg.temperature
        )
...
def _backward_kl(
    model, ref_model, groups, cfg, real_vocab, device, pad_id, micro_batch: int
) -> float:
...
def main() -> int:
    ...
    rmiss, runexp = ref.load_state_dict(rck.get("model_state_dict", rck), strict=False)
    ...
    print(
        f"  kl_coeff={cfg.kl_coeff} SCORING temperature={cfg.temperature} "
    ...
    model.zero_grad(set_to_none=True)
    live_c, changed = _backward_policy(
        model,
        ref,
        groups,
        cfg,
    ...
    model.zero_grad(set_to_none=True)
    mean_kl = _backward_kl(
        model, ref, groups, cfg, mc.real_vocab_size, device, pad_id, args.micro_batch
    )
    ...
    print(
        f"  live clamped   {live_c}/{n_roll} ({live_c / n_roll:.1%})"
        f"   delta {live_c - live_u:+d}"
    )
    print(
        f"  changed rollouts (wrong, adv>0 -> 0): {changed} "
    ...
    for grp in ("all", "base", "hra"):
        print(
            f"  ||beta*KL|| / ||policy_unclamped||  [{grp}] = {nk[grp] / nu[grp]:.3f}"
            if nu[grp] > 0
            else ""
        )
    ...
    for label, pk in (("unclamped", "policy_unclamped"), ("clamped", "policy_clamped")):
        ...
        print(
            f"  {label:<10} ||policy|| {np_:.4f}  ||beta*KL|| {nk:.4f}  "
            f"||combined|| {n_comb:.4f}"
        )
        print(
            f"  {'':<10} KL rotates the update {ang:.1f} deg off the policy "
            f"direction; clip coef (max_norm {cfg.grad_clip}) = {clip:.4f}"
        )
    print(
        "  NOTE: raw-gradient geometry only. Translating this into parameter"
