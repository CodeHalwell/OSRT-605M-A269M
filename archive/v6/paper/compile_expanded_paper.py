...
\end{equation}
where $W_{\text{kv\_down}} \in \mathbb{R}^{1536 \times 512}$. During generation, only $\mathbf{c}_{\text{kv}}$ is stored in the KV cache, requiring 512 scalar allocations per token instead of $8 \times 64 \times 2 = 1024$ scalars under a Grouped-Query Attention (GQA) baseline.

At attention computation, Keys ($K$) and Values ($V$) are reconstructed from $\mathbf{c}_{\text{kv}}$:
\begin{align}
K &= \text{view}(\mathbf{c}_{\text{kv}}) \cdot W_{\text{K\_proj}} \\
V &= \mathbf{c}_{\text{kv}} W_{\text{v\_from\_k}}
\end{align}
where $W_{\text{v\_from\_k}} \in \mathbb{R}^{512 \times 512}$ with a bias vector. The queries $Q$ are projected from the hidden state. 
...
subprocess.run([
    "qlmanage", "-t", "-s", "1000", "-o", "docs/diagrams", "docs/diagrams/05-recursion.svg"
], check=True)

src_png = "docs/diagrams/05-recursion.svg.png"
dst_png = "docs/diagrams/05-recursion.png"
if os.path.exists(src_png):
    if os.path.exists(dst_png):
        os.remove(dst_png)
    os.rename(src_png, dst_png)
    print("Successfully created docs/diagrams/05-recursion.png")
else:
    print(f"Error: {src_png} was not generated!")
