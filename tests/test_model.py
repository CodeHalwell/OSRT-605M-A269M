...
def test_hybrid_optimizer_step_updates_both_kinds_of_params():
    ...
    q_weight_before = model.model.blocks[0].q_proj.weight.detach().clone()
...
def test_num_loops_reduced_produces_finite_logits_right_shape():
    ...
    for k in (1, 2, 3, 4):
        ...
        assert torch.isfinite(out.logits).all(), f"non-finite logits at num_loops={k}"
...
def test_num_loops_reduced_kv_cache_consistent_decode():
    ...
    with torch.no_grad():
        ...
        out_dec = model(
            full[:, 6:7],
            past_key_values=out_pre.past_key_values,
            use_cache=True,
            num_loops=k,
        )
...
def test_speculative_matches_greedy_when_draft_equals_full():
    ...
    torch.manual_seed(0)
    cfg = tiny_config(
        recursive_loops=3, spec_draft_loops=3, router_capacity_factor=10.0
    )
    model = OSRTForCausalLM(cfg)
    model.train(False)
...
def test_speculative_matches_greedy_general_draft_loops():
    ...
    torch.manual_seed(0)
    cfg = tiny_config(
        recursive_loops=4, spec_draft_loops=2, router_capacity_factor=10.0
    )
    model = OSRTForCausalLM(cfg)
    model.train(False)
...
def test_speculative_respects_num_loops_cap():
    ...
    torch.manual_seed(0)
    cfg = tiny_config(
        recursive_loops=4, spec_draft_loops=3, router_capacity_factor=10.0
    )
    model = OSRTForCausalLM(cfg)
    model.train(False)
    ctx = torch.randint(0, cfg.vocab_size, (1, 6))
