...
def pull_latest(
    repo_id: str, ckpt_dir: str, prefix: str, base_name: str | None = None
) -> None:
    ...
    try:
        ...
        print(f"[hf-sync] no repo yet ({type(e).__name__}); starting clean", flush=True)
        files = []

    if base_name and not os.path.exists(os.path.join(ckpt_dir, base_name)):
        if base_name in files:
            print(f"[hf-sync] pulling base {base_name}...", flush=True)
            hf_hub_download(repo_id, base_name, repo_type="model", local_dir=ckpt_dir)
    ...
    steps = [f for f in files if re.match(rf"{prefix}_(?:rescue_)?step_\d+\.pt$", f)]
    if steps:
        ...
        print("[hf-sync] no prior checkpoints in repo — starting from base", flush=True)


def start_push_daemon(
    repo_id: str, ckpt_dir: str, prefix: str, interval: int = 60, keep_remote: int = 3
) -> None:
        ...
        while True:
            try:
                local = sorted(
                    ...
                    + glob.glob(os.path.join(ckpt_dir, f"{prefix}_rescue_step_*.pt")),
                    key=_step_of,
                )
                for path in local:
                ...
                print(f"[hf-sync] WARN {type(e).__name__}: {str(e)[:120]}", flush=True)
            time.sleep(interval)

    threading.Thread(target=_loop, daemon=True).start()
    print(
        f"[hf-sync] push daemon started → {repo_id} (every {interval}s, "
        f"keep {keep_remote})",
        flush=True,
    )
...
def flush(repo_id: str, ckpt_dir: str, prefix: str) -> None:
    ...
    try:
        ...
        print(
            f"[hf-sync] flush: cannot list repo ({type(e).__name__}); skipping",
            flush=True,
        )
        return
    ...
    for path in sorted(set(local), key=_step_of):
        ...
        try:
            ...
            print(
                f"[hf-sync] flush WARN {type(e).__name__}: {str(e)[:120]}", flush=True
            )
